...
$query = "SELECT id, name, email, role, experience_level, created_at FROM users WHERE 1=1";
...
<head>
   ...
   <title>Manage Users - EliteFit Gym</title>
...
<body>
   <div class="dashboard-container">
       ...
       <div class="main-content">
           ...
           <div class="card">
               <form action="users.php" method="get" id="filterForm">
                   <div class="filter-section">
                       <div class="filter-group">
                           ...
                           <select name="role" id="role" class="filter-select">
                               ...
                               <option value="EquipmentManager" <?php echo $role === 'EquipmentManager' ? 'selected' : ''; ?>>Equipment Manager</option>
                           </select>
                       </div>
                       ...
                       <a href="users.php" class="btn btn-sm" style="background-color: var(--secondary);">Reset</a>
                   </div>
               </form>
           </div>
                       ...
                       <?php if (count($users) > 0): ?>
                           <?php foreach ($users as $user): ?>
                               <tr>
                                   ...
                                   <td><?php echo htmlspecialchars($user['experience_level'] ?? 'N/A'); ?></td>
                                   <td><?php echo date('M d, Y', strtotime($user['created_at'])); ?></td>
                                   <td>
                                       <a href="view-user.php?id=<?php echo $user['id']; ?>" class="btn btn-sm btn-info view-user" data-id="<?php echo $user['id']; ?>" title="View"><i class="fas fa-eye"></i></a>
                                       ...
                                       <?php if ($user['role'] !== 'Admin' || $user['id'] !== $userId): ?>
                                           <a href="javascript:void(0);" onclick="confirmDelete(<?php echo $user['id']; ?>, '<?php echo htmlspecialchars($user['name']); ?>')" class="btn btn-sm btn-danger delete-user" data-id="<?php echo $user['id']; ?>" title="Delete"><i class="fas fa-trash"></i></a>
                                       <?php endif; ?>
                                   </td>
                               </tr>
                           <?php endforeach; ?>
           ...
           <?php if ($totalPages > 1): ?>
               <div class="pagination">
                   <?php if ($page > 1): ?>
                       <a href="?page=<?php echo $page - 1; ?>&role=<?php echo urlencode($role); ?>&search=<?php echo urlencode($search); ?>">Previous</a>
                   <?php else: ?>
                   ...
                   <?php for ($i = 1; $i <= $totalPages; $i++): ?>
                       ...
                       <?php else: ?>
                           <a href="?page=<?php echo $i; ?>&role=<?php echo urlencode($role); ?>&search=<?php echo urlencode($search); ?>"><?php echo $i; ?></a>
                       <?php endif; ?>
                   <?php endfor; ?>
                   
                   <?php if ($page < $totalPages): ?>
                       <a href="?page=<?php echo $page + 1; ?>&role=<?php echo urlencode($role); ?>&search=<?php echo urlencode($search); ?>">Next</a>
                   <?php else: ?>
