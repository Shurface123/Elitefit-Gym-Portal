...
$query = "SELECT id, name, email, experience_level AS expertise, fitness_goals AS experience, preferred_routines AS certifications, created_at FROM users WHERE role = 'EquipmentManager'";
$countQuery = "SELECT COUNT(*) as total FROM users WHERE role = 'EquipmentManager'";
$params = [];
...
// Get all expertise areas for filter dropdown
$expertiseStmt = $conn->prepare("SELECT DISTINCT experience_level FROM users WHERE role = 'EquipmentManager' AND experience_level IS NOT NULL ORDER BY experience_level");
$expertiseStmt->execute();
$expertiseAreas = $expertiseStmt->fetchAll(PDO::FETCH_COLUMN);
...
<head>
    ...
    <title>Manage Equipment Managers - EliteFit Gym</title>
...
<body>
    <div class="dashboard-container">
        ...
        <div class="sidebar">
            ...
            <ul class="sidebar-menu">
                ...
                <li><a href="equipment-managers.php" class="active"><i class="fas fa-dumbbell"></i> <span>Equipment</span></a></li>
        ...
        <div class="main-content">
            ...
            <div class="header">
                <div>
                    <h1>Manage Equipment Managers</h1>
                    <p>View and manage all equipment managers in the system</p>
                </div>
                <div class="user-info">
            ...
            <div class="card">
                <form action="" method="get">
                    <div class="filter-section">
                        <div class="filter-group">
                            <label class="filter-label" for="expertise">Area of Expertise:</label>
                            <select name="expertise" id="expertise" class="filter-select">
                                <option value="">All Areas</option>
                                <?php foreach ($expertiseAreas as $area): ?>
                                    <option value="<?php echo htmlspecialchars($area); ?>" <?php echo $expertise === $area ? 'selected' : ''; ?>>
                        ...
                        <a href="equipment-managers.php" class="btn btn-sm" style="background-color: var(--secondary);">Reset</a>
                    </div>
                </form>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <span class="badge badge-warning">
                                                <?php echo htmlspecialchars($manager['expertise'] ?? 'N/A'); ?>
                                            </span>
                                        </td>
                                        <td class="truncate" title="<?php echo htmlspecialchars($manager['experience'] ?? ''); ?>">
                                            <?php echo htmlspecialchars($manager['experience'] ?? 'N/A'); ?>
                                        </td>
                                        <td><?php echo date('M d, Y', strtotime($manager['created_at'])); ?></td>
                                        <td>
                                            <a href="view-equipment-manager.php?id=<?php echo $manager['id']; ?>" class="btn btn-sm btn-info" title="View"><i class="fas fa-eye"></i></a>
                                            <a href="edit-user.php?id=<?php echo $manager['id']; ?>" class="btn btn-sm btn-warning" title="Edit"><i class="fas fa-edit"></i></a>
                                            <a href="javascript:void(0);" onclick="confirmDelete(<?php echo $manager['id']; ?>, '<?php echo htmlspecialchars($manager['name']); ?>')" class="btn btn-sm btn-danger" title="Delete"><i class="fas fa-trash"></i></a>
                                        </td>
                                    </tr>
                                <?php endforeach; ?>
                            <?php else: ?>
                                <tr>
                                    <td colspan="7" style="text-align: center;">No equipment managers found.</td>
                                </tr>
                            <?php endif; ?>
                        </tbody>
                    </table>
                ...
                <?php if ($totalPages > 1): ?>
                    <div class="pagination">
                        <?php if ($page > 1): ?>
                            <a href="?page=<?php echo $page - 1; ?>&expertise=<?php echo urlencode($expertise); ?>&search=<?php echo urlencode($search); ?>">Previous</a>
                        <?php else: ?>
                        ...
                        <?php for ($i = 1; $i <= $totalPages; $i++): ?>
                            ...
                            <?php else: ?>
                                <a href="?page=<?php echo $i; ?>&expertise=<?php echo urlencode($expertise); ?>&search=<?php echo urlencode($search); ?>"><?php echo $i; ?></a>
                            <?php endif; ?>
                        <?php endfor; ?>
                        
                        <?php if ($page < $totalPages): ?>
                            <a href="?page=<?php echo $page + 1; ?>&expertise=<?php echo urlencode($expertise); ?>&search=<?php echo urlencode($search); ?>">Next</a>
                        <?php else: ?>
    ...
    <div id="deleteModal" style="display: none; position: fixed; z-index: 1050; left: 0; top: 0; width: 100%; height: 100%; overflow: auto; background-color: rgba(0,0,0,0.4);">
        <div style="background-color: var(--card-bg); margin: 15% auto; padding: 20px; border: 1px solid var(--border-color); border-radius: var(--border-radius); width: 50%; max-width: 500px; box-shadow: 0 4px 8px rgba(0,0,0,0.1);">
            ...
            <p style="margin-bottom: 20px; color: var(--text-color);">Are you sure you want to delete equipment manager <span id="deleteUserName"></span>? This action cannot be undone.</p>
